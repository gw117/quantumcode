...
var stringrand  = app.ReadFile("/sdcard/seed.txt");
    ...
    btn2= app.CreateButton("bet low", 0.2, 0.1);
    btn2.SetOnTouch(betlow);
    btn2.SetMargins(0.6, 0.02, 0, 0);
    lay.AddChild(btn2);
    
    
    btn3 = app.CreateButton("bet high", 0.2, 0.1);
    btn3.SetOnTouch(bethigh);
    btn3.SetMargins(0.4, 0.02, 0, 0);
    lay.AddChild(btn3);
...
function btns_OnTouch() { //initiate camera stream function
}
...
function OnMotion(data) //stream camera data
{
    ...
    if (or > corr) {
        ...
        if (check[cyc] != qu) { //this section checks for disruptions in quantum flux, it is the time the code is most sure of inference/disruption(unless you turn the device off),why? because we cause the disruption at a future time by adding 9s to 'numa',you may wonder many things, you may aswell throw computer logic out the window because this is essentially completely made up code from here on
            ...
            and = 0;
            cyc++;
            app.ShowPopup("Bet low " + ghostprotocol);
            prime++;
            if (prime > 1 && ghostprotocol > 3) {
               ...
               qdata += parseInt(resstring);//no way i am sorting out qdata notation fixing, because it doesn't output the correct information all the time in reliable terms,its more like a probabilistic nightmare to calculate with reducing notations and i think those possibilities interferes with the overall results, who knows.
                 ghostprotocol = 0; //i have it setup to detect just one segmentation, it needs to detect many.
                       ...
                       if (range > 0){
                          // range = (range*10);// or use divide to reduce
                           }
                    edtq.SetText(outputq);
...
var command = "a=PlaceBet&s=2a57e75bc9c04c2a90b8f4c986fa8e9a&PayIn="+ PayIn + "&Low=0&High=500000&ClientSeed=-5309338182208444527&currency=doge&ProtocolVersion=2"
...
function bethigh() {
var command = "a=PlaceBet&s=2a57e75bc9c04c2a90b8f4c986fa8e9a&PayIn="+ PayIn + "&Low=500000&High=999999&ClientSeed=-5309338182208444527&currency=doge&ProtocolVersion=2"
    httpRequest = new XMLHttpRequest();    
    httpRequest.open("POST", url, true); 
    httpRequest.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    httpRequest.onreadystatechange = function() { HandleReply(httpRequest); };
...
app.SetClipboardText(input );
    ...
    integer = 100 ;//or set just a decimal instead of converting data and crc into a decimal
    
    /*
    //do quantum physics
    if (integer <= (ghostprotocol * range) + qdata) { //do quantum magic to determine decimal
        for (var a = 0; a < 500; a++) {
            var b = 9;
